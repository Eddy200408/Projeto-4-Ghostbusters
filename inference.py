...
def constructBayesNet(gameState: hunters.GameState):
    ...
    # Define as conexões entre as variáveis
    edges = [
        (PAC, OBS0), (PAC, OBS1),
        (GHOST0, OBS0), (GHOST1, OBS1)
    ]

    # Define os domínios das variáveis
    positions = [(x, y) for x in range(X_RANGE) for y in range(Y_RANGE)]
    max_distance = (X_RANGE - 1) + (Y_RANGE - 1)  # Distância Manhattan máxima no tabuleiro
    observed_distances = list(range(max_distance + 1 + MAX_NOISE))  # Inclui ruído máximo

    variableDomainsDict = {
        PAC: positions,
        GHOST0: positions,
        GHOST1: positions,
        OBS0: observed_distances,
        OBS1: observed_distances,
    }
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        # Obter os fatores considerando as evidências
        currentFactorsList = bayesNet.getAllCPTsWithEvidence(evidenceDict)
...
class DiscreteDistribution(dict):
    ...
    def total(self):
        """
        Return the sum of all values in the distribution.
        """
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        """
        "*** YOUR CODE HERE ***"
        # Caso especial: o fantasma está na prisão
        if ghostPosition == jailPosition:
            return 1.0 if noisyDistance is None else 0.0

        # Caso especial: a observação é None, mas o fantasma não está na prisão
        if noisyDistance is None:
            return 0.0

        # Calcula a distância real entre o Pacman e o fantasma
        trueDistance = manhattanDistance(pacmanPosition, ghostPosition)

        # Retorna a probabilidade baseada na distância real e observada
